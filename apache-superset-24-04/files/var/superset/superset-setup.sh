...
echo "This script will setup Apache Superset SSL"
echo "--------------------------------------------------"
...
echo "--------------------------------------------------"
echo "Enter the domain name for your new Apache Superset site."
...
sed -i "s/superset.example.com/$dom/g" /var/superset/superset_ssl
rm -rf /etc/nginx/sites-available/superset
cp -rf /var/superset/superset_ssl /etc/nginx/sites-available/superset

service nginx start

# Remove the setup from bashrc so it does not run again
