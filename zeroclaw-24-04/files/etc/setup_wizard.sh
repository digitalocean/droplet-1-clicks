...
write_gradient_env_key() {
  ...
  umask 077
  touch "$ENV_FILE"
  grep -Ev '^(GRADIENT_KEY|GRADIENT_MODEL|DO_INFERENCE_ROUTER)=' "$ENV_FILE" >"${ENV_FILE}.tmp" 2>/dev/null || : >"${ENV_FILE}.tmp"
...
do
  case $opt in
    "DigitalOcean Gradient")
        ...
        gradient_options=("Kimi K2.5" "MiniMax M2.5" "GLM 5" "Claude Sonnet 4.5" "Intelligent Inference Router")
...
if [[ "$onboard_provider" == "custom:https://inference.do-ai.run/v1" ]]; then
  write_gradient_env_key "$model_access_key" "$onboard_model" "$onboard_router"
