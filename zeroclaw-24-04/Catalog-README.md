...
2. **Configure inference** using one of:
   - Set `GRADIENT_KEY` (and optional `GRADIENT_MODEL`) as droplet environment variables at create time — Serverless Inference is applied on first boot with no wizard
...
| Variable | Purpose |
|----------|---------|
| `GRADIENT_KEY` | DigitalOcean model access key (required for auto-config) |
