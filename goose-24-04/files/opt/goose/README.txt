...
The first time you log in as root, /opt/goose/first-login-setup.sh runs automatically.
  ...
  3. Goose CLI is installed when this Marketplace snapshot is built (not on first
     ...
     interactive provider setup beyond DigitalOcean Inference.

  4. DigitalOcean Inference AI (optional): the wizard copies a custom Goose
     provider to ~/.config/goose/custom_providers/ and can store your DigitalOcean Inference
     model access key in:
       - ~/.config/goose/secrets.yaml (with GOOSE_DISABLE_KEYRING so headless
         Droplets do not require a desktop secret service),
       - /etc/profile.d/goose-do.sh and a small block in ~/.bashrc so
         DO_MODEL_ACCESS_API_KEY is set even for non-login SSH shells.
     Default Goose provider id is digitalocean_model with default model minimax-m2.5;
     you do not need to run "goose configure" for DigitalOcean Inference alone.
     In the Goose UI, pick DigitalOcean Inference (provider id digitalocean_model).
     To add or change the key later:

       /opt/goose/configure-do-key.sh

     Create a model access key in the DO control panel (Gen AI / DigitalOcean Inference).
     ...
     workaround) or your declarative JSON is stale, run as root:
       ...
       /opt/goose/migrate-do-provider-id.sh

     That refreshes digitalocean_model.json from /opt/goose and rewrites
     GOOSE_PROVIDER to digitalocean_model when it was kimi. Re-running
     /opt/goose/configure-do-key.sh does the same migration before the key prompt.

     If that script is not on your Droplet yet, set GOOSE_PROVIDER to
     digitalocean_model in ~/.config/goose/config.yaml and replace
     ~/.config/goose/custom_providers/digitalocean_model.json with the
     version from this 1-Click (the "name" field inside the JSON must be
     digitalocean_model).

CHANGE DIGITALOCEAN INFERENCE MODEL (e.g. minimax-m2.5 to kimi-k3)
======================================================

  Edit /root/.config/goose/config.yaml and set GOOSE_MODEL to one of the "name"
  values in /root/.config/goose/custom_providers/digitalocean_model.json
  (for example kimi-k3, glm-5.3, anthropic-claude-4.5-sonnet). Keep
  GOOSE_PROVIDER as digitalocean_model. Or run "goose configure" and choose
  another model when offered. Rotating the API key with
  /opt/goose/configure-do-key.sh does not overwrite GOOSE_MODEL if it is
  already set.

  Note: kimi-k3 can be slow with some Goose builds on DigitalOcean Inference (max_tokens vs
  max_completion_tokens). Prefer minimax-m2.5 as default; upstream Goose can add
  digitalocean_model to its OpenAI compat remap list for a full fix.
