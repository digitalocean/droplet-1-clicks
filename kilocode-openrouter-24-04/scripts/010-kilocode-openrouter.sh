...
chmod +x /opt/setup-kilocode-openrouter.sh
chmod +x /opt/update-kilocode.sh
chmod +x /etc/update-motd.d/99-one-click
chmod +x /var/lib/cloud/scripts/per-instance/001_onboot

# Shipped OpenRouter attribution config (leave contents alone)
mkdir -p /root/.config/kilo
chmod 700 /root/.config/kilo
chmod 600 /root/.config/kilo/kilo.jsonc /root/.config/kilo/kilo.json 2>/dev/null || true

echo "Kilo Code CLI (OpenRouter) installation complete."
