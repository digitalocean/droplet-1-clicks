#!/bin/bash
# Approve pending Control UI pairing (non-interactive). For the full browser
# walkthrough use: sudo /opt/openclaw-control-ui-pairing.sh
#
...
GATEWAY_TOKEN=$(grep -E '^OPENCLAW_GATEWAY_TOKEN=' /opt/openclaw.env 2>/dev/null | tail -n 1)
GATEWAY_TOKEN="${GATEWAY_TOKEN#OPENCLAW_GATEWAY_TOKEN=}"
GATEWAY_TOKEN="${GATEWAY_TOKEN%\"}"
GATEWAY_TOKEN="${GATEWAY_TOKEN#\"}"
GATEWAY_TOKEN="${GATEWAY_TOKEN%\'}"
GATEWAY_TOKEN="${GATEWAY_TOKEN#\'}"
if [ -z "$GATEWAY_TOKEN" ] || [[ "$GATEWAY_TOKEN" == *PLACEHOLDER* ]]; then
  if [ -f /home/openclaw/.openclaw/openclaw.json ]; then
    GATEWAY_TOKEN=$(jq -r '.gateway.auth.token // .gateway.remote.token // empty' /home/openclaw/.openclaw/openclaw.json 2>/dev/null)
  fi
fi
