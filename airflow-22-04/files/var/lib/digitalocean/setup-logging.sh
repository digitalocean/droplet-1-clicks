...
if [ -f "/root/.digitalocean_spaces_credentials" ] && grep -q '^spaces_conn_type="aws"$' "/root/.digitalocean_spaces_credentials"; then
     ...
     sed -i -e "s/^remote_logging\s*=.*/remote_logging = True/" \
          -e "s/^remote_log_conn_id\s*=.*/remote_log_conn_id = $SPACES_CONN_ID/" \
          -e "s|^remote_base_log_folder\s*=.*|remote_base_log_folder = s3://$S3_BUCKET/logs|" \
     ...
     CONN_EXTRA=$(jq -n --arg S3_REGION "$S3_REGION" --arg S3_HOST "$S3_HOST" '{"ACL":"private", "region_name":$S3_REGION, "endpoint_url":$S3_HOST, "s3_config_kwargs":{"addressing_style":"path"}}')
     CONN=\'$CONN_EXTRA\'
